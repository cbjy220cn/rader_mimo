...
% 孔径扩展参数
N_L = 4;                            % 扩展因子（增大以实际进行孔径扩展）
N_num = 2*(N_L-1)+1;                % 扩展后的等效阵元数 = 7

fprintf('  接收阵列: %d元UCA, 半径 %.2f cm\n', numRX, R_rx*100);
fprintf('  孔径扩展因子 N_L: %d (等效 %d 个位置)\n', N_L, N_num);
fprintf('  扩展后等效阵元数: %d × %d = %d\n', numRX, N_num, numRX*N_num);
...
% ===== 定位误差 =====
% 位置误差标准差 (单位: 米)
% 波长λ ≈ 10cm，误差范围限制在1λ以内
% 典型GPS误差: 1-5 cm (RTK)
position_error_std_list = [0, 0.005, 0.01, 0.02, 0.05, 0.10];  % 0, 0.5cm, 1cm, 2cm, 5cm, 10cm (≈1λ)
fprintf('  位置误差标准差: ');
fprintf('%.1fcm ', position_error_std_list * 100);
fprintf('\n');
fprintf('  (波长λ=%.1fcm，最大误差≈1λ)\n', 10.0);

% 姿态角误差标准差 (单位: 度)
% 典型IMU误差: 0.1-1°
attitude_error_std_list = [0, 0.5, 1.0, 2.0, 3.0, 5.0];  % 0, 0.5°, 1°, 2°, 3°, 5°
fprintf('  姿态角误差标准差: ');
...
for pos_idx = 1:length(position_error_std_list)
    ...
    for snr_idx = 1:length(snr_list)
        ...
        fprintf('  位置误差=%.1fcm, SNR=%ddB: RMSE_θ=%.2f°, RMSE_φ=%.2f°\n', ...
            pos_error_std*100, snr, ...
...
% 选择中等误差进行对比
test_pos_error = 0.02;  % 2cm位置误差 (≈0.2λ)
test_att_error = 1.0;   % 1°姿态误差
test_snr = 20;          % 20dB SNR

fprintf('测试条件: 位置误差=%.1fcm (≈%.1fλ), 姿态误差=%.1f°, SNR=%ddB\n\n', ...
    test_pos_error*100, test_pos_error/0.1, test_att_error, test_snr);
...
markers = {'o', 's', 'd', '^'};
for snr_idx = 1:length(snr_list)
    plot(position_error_std_list * 100, results_pos_error.rmse_phi(:, snr_idx), ...
...
xlabel('位置误差标准差 (cm)', 'FontWeight', 'bold');
ylabel('方位角RMSE (°)', 'FontWeight', 'bold');
title('(a) 位置误差对方位角估计的影响', 'FontWeight', 'bold');
legend('Location', 'northwest');
grid on;
xlim([0, max(position_error_std_list)*100*1.1]);

subplot(1, 2, 2);
hold on;
for snr_idx = 1:length(snr_list)
    plot(position_error_std_list * 100, results_pos_error.rmse_theta(:, snr_idx), ...
        ['-' markers{snr_idx}], 'Color', colors(snr_idx,:), ...
        'LineWidth', 2, 'MarkerSize', 8, 'MarkerFaceColor', colors(snr_idx,:), ...
        'DisplayName', sprintf('SNR=%ddB', snr_list(snr_idx)));
end
...
title(sprintf('定位误差条件下的鲁棒性对比\n(位置误差=%.0fcm, 姿态误差=%.1f°, SNR=%ddB)', ...
    test_pos_error*100, test_att_error, test_snr), 'FontWeight', 'bold');
...
fprintf('【位置误差影响】\n');
fprintf('  - 位置误差从0增加到%.0fcm时:\n', max(position_error_std_list)*100);
...
fprintf('\n【姿态误差影响】\n');
fprintf('  - 姿态误差从0增加到%.0f°时:\n', max(attitude_error_std_list));
fprintf('    方位角RMSE增加: %.2f° → %.2f° (SNR=20dB)\n', ...
